...
// Utiliser le registre comme source unique de vérité (plus de liste hardcodée)
$registry_languages = LinguaCommerce_Language_Registry::get_all();
$all_languages = array();
foreach ( $registry_languages as $code => $data ) {
    $is_rtl = in_array( substr( $code, 0, 2 ), array( 'ar', 'he', 'fa', 'ur' ) );
    $all_languages[] = array(
        'code'   => $code,
        'name'   => $data['name'],
...
// Trier par nom natif
usort( $all_languages, function( $a, $b ) {
...
// Récupérer la langue source depuis la BDD (source unique de vérité)
$source_lang_row = $wpdb->get_row( "SELECT code FROM {$table_languages} WHERE is_default = 1 LIMIT 1" );
$source_lang = $source_lang_row ? $source_lang_row->code : 'fr_FR';
